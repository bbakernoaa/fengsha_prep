...
def mock_soil_data(mock_geo_data: dict) -> xr.Dataset:
    ...
    ds["bdod"] = xr.DataArray(
        np.full((10, 10), 1.4), dims=mock_geo_data["dims"]
    )  # Added bdod
    return ds
...
def test_generate_dust_flux_map(
    mock_albedo_data,
    mock_lai_data,
    mock_lc_data,
    mock_soil_data,
    ...
    with patch(
        "fengsha_prep.pipelines.uthresh.core.compute_hybrid_drag_partition",
        return_value=mock_r,
    ):
        with patch(
            "fengsha_prep.pipelines.uthresh.core.compute_moisture_inhibition",
            return_value=mock_h,
        ):
