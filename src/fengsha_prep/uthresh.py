...
def compute_moisture_inhibition(
    moisture: Union[float, np.ndarray], clay: Union[float, np.ndarray], soc: Union[float, np.ndarray]
...
def generate_dust_flux_map(
    ...
    # Saltation Q (g/m/s)
    Q = xr.where(u_eff > u_thresh, (1.23/9.81)*u_eff**3 * (1-u_thresh**2/u_eff**2)*(1+u_thresh/u_eff), 0)
    # Sandblasting alpha
